    ...
    $.fn.aviaSlider = function(variables) {
        ...
        return this.each(function() {
            ...
            slideWrapper.methods = {
                ...
                appendControlls : function() {
                    if (options.slideControlls == 'items') {
                        ...
                        $(document).ready(function(){
                            $('#carousel-controls').children('.carousel-button').remove();
                        })

                        controlls = $('<div></div>').addClass('slidecontrolls').appendTo('#carousel-controls');
                        slides.each(function(i) {
                            var controller = $('<a href="#" class="ie6fix carousel-button ' + current_class + '"></a>').appendTo(controlls);
                ...
                preloadingDone : function() {   // executed only once
                    skipSwitch = false;
                    if (/*$.browser.msie*/ true === true) {
                        slides.css({
                ...
                autorotate : function() {
                    var timeNum = parseInt(options.autorotationSpeed) /** 1000*/ + (options.betweenBlockDelay * blockNumber) + options.animationSpeed;
                ...
                switchSlide : function(passed) {
                    ...
                    if (!skipSwitch && noAction == false) {
                        skipSwitch = true;
                        var currentSlide = slides.filter(':visible'), nextSlide = slides.filter(':eq(' + currentSlideNumber + ')'), nextURL = $.data(nextSlide[0], "data").img, nextImageBG = 'url(' + nextURL + ')';
                        if (options.slideControlls) {
                            controlls.find('.active-button').removeClass('active-button').addClass('inactive-button');
                            controlls.find('a:eq(' + currentSlideNumber + ')').addClass('active-button').removeClass('inactive-button');
                        }
                        ...
                        if (options.switchMovement && (options.display == "topleft" || options.display == "diagonaltop")) {
                            if (reverseSwitch == false) {
                                ...
                                if (options.display == "topleft")
                                    blockSelectionJQ = blockOrder['bottomright'];
                                if (options.display == "diagonaltop")
                        ...
                        if (options.display == 'all') {
                            ...
                            currentTransition++;
                            if (currentTransition >= options.transitionOrder.length)
                                currentTransition = 0;
                        }
                ...
                fyrandomize : function(object) {
                    var length = object.length, objectSorted = $(object);
                    if (length == 0)
                        return false;
                    while (--length) {
                        var newObject = Math.floor(Math.random() * (length + 1)), temp1 = objectSorted[length], temp2 = objectSorted[newObject];
                ...
                kcubit : function(object) {
                    var length = object.length, objectSorted = $(object), currentIndex = 0, rows = Math.ceil(slideHeight / options.blockSize.height), columns = Math.ceil(slideWidth / options.blockSize.width), oneColumn = blockNumber / columns, oneRow = blockNumber / rows, modX = 0, modY = 0, i = 0, rowend = 0, endreached = false, onlyOne = false;
                    if (length == 0)
                        return false;
                    for ( i = 0; i < length; i++) {
...
(function($) {
    $.fn.aviaSlider_preloadhelper = function(variables, callback) {
        ...
        var options = $.extend(defaults, variables);
        return this.each(function() {
            var container = $(this), images = $('img', this).css({
                // opacity : 0,
                // visibility : 'hidden',
                display : 'block'
            }), parent = images.parent(), imageCount = images.length, interval = '', allImages = images;
            var methods = {
                checkImage : function() {
